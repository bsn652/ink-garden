    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-ink-black">
        <div className="text-center">
          ...
          <Link href="/articles" className="text-white/40 hover:text-white transition-colors text-sm">← 返回文章列表</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <article className="max-w-3xl mx-auto px-6 py-16">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3 }} className="prose">
          <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
            ...
            blockquote: ({ children, ...props }) => <blockquote className="border-l-2 border-ink-black/20 pl-6 italic text-ink-dark/80 my-8" {...props}>{children}</blockquote>,
            pre: ({ children, ...props }) => <pre className="bg-ink-black/5 rounded-lg p-5 overflow-x-auto my-6 text-sm" {...props}>{children}</pre>,
            code: ({ children, ...props }) => <code className="bg-ink-black/5 px-1.5 py-0.5 rounded text-sm font-mono" {...props}>{children}</code>,
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {post.tags.map((tag) => (
                    <Link key={tag} href={`/tags?tag=${encodeURIComponent(tag)}`}
                      className="text-xs px-3 py-1 rounded-full bg-ink-black/5 text-ink-dark/50 hover:bg-ink-black/10 transition-colors">{tag}</Link>
                  ))}
                </div>
              </div>
              <Link href="/articles" className="text-xs text-ink-dark/40 hover:text-ink-black transition-colors flex items-center gap-1 group">
